...
import {messengerType, redisConstants, socketEvents} from "../common/constants";
import {IOnlineUser} from "./interfaces/online-users.interface";
import {extractSocketCodeFromKeys, filterSocketUserInfo, removeUsersBySocketCode} from "../common/utils";
import {User} from "../user/schema/user-schema";
import {IMessage, MessengerType, RoomEnum} from "./interfaces/message.interface";
...
export class ChatService extends GenericService<ChatMessage> {
    ...
    async addOnlineUser(user: User, clientId: string): Promise<IOnlineUser[]> {
        ...
        const users = await this.cacheManager.store.get(redisConstants.USER_LIST) as Array<IOnlineUser>
        const updatedUsers = users?.length > 0 ? [...users, onlineUser] : [onlineUser]
    ...
    async removeSocketFromOnlineList(socketCodes: string[]) {
        let onlineUsers = await this.cacheManager.store.get(redisConstants.USER_LIST) as Array<IOnlineUser>
    ...
    async getInfoFromOnlineUsers(userId: string) {
        const onlineUsers = await this.cacheManager.store.get(redisConstants.USER_LIST) as Array<IOnlineUser>
        return filterSocketUserInfo(userId, onlineUsers)
    }

    async messageToRoom(chatMessage: IMessage, socket: Server, authUser: User) {
        if (chatMessage.receiverId === RoomEnum.principal) {
            const usersInRoom = await this.cacheManager.store.keys(`${RoomEnum.principal}*`)
            const result = extractSocketCodeFromKeys(usersInRoom)
            result.forEach(value => socket.emit(socketEvents.message,value))
        }
    }

    async messageToUser(chatMessage: IMessage, connectedUsers: Socket[], authUser: User) {

        const onlineUsers = await this.getInfoFromOnlineUsers(chatMessage.receiverId)
